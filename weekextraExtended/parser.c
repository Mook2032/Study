...
void compileProgram(void) {
  ...
  program->progAttrs->codeAddress = newLabel();
  //genJ(program->progAttrs->codeAddress); <~ the translator will automatically generate this.
  //No, please, don't generate it by hand, or something really bad will happen!
  //...I think.
...
void compileBlock(void) {
  ...
  Object* obj = symtab->currentScope->owner;
  switch (obj->kind)
  {
	case OBJ_PROGRAM: genLABEL(obj->progAttrs->codeAddress); break;
	case OBJ_FUNCTION: genLABEL(obj->funcAttrs->codeAddress); break;
	case OBJ_PROCEDURE: genLABEL(obj->procAttrs->codeAddress); break;
	default: break;
  }
  genENTER();
...
void compileFuncDecl(void) {
  ...
  funcObj->funcAttrs->codeAddress = newLabel();
...
void compileProcDecl(void) {
  ...
  procObj->procAttrs->codeAddress = newLabel();
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_FLOAT:
    eat(TK_FLOAT);
    constValue = makeFloatConstant(currentToken->valueFloat);
  default:
...
void compileForSt(void) {
  ...
  if ((compareType(varType,intType)==0)&&(compareType(varType,charType)==0))
	error(ERR_TYPE_UNUSABLE, currentToken->lineNo, currentToken->colNo);
  eat(SB_ASSIGN);
...
void compileCondition(void) {
  ...
  if (compareType(type1,stringType) == 1)
  {
	  switch (op) {
	  case SB_EQ:
		genEQS();
  ...
  else if (compareType(type1,floatType) == 1)
  {
	  switch (op) {
	  case SB_EQ:
		genEQF();
...
Type* compileExpression2(void) {
  ...
  type = compileTerm();
  if (compareType(type, stringType)==1)
	return type;
...
Type* compileExpression3(Type* argType1) {
  ...
  case SB_PLUS:
	...
	if (compareType(argType1,floatType) == 1)
		genADF();
	else genAD();
...
Type* compileFactor(void) {
  ...
  case TK_FLOAT:
    eat(TK_FLOAT);
	type = floatType;
	genLCF(currentToken->valueFloat);
    break;
  case TK_STRING:
    eat(TK_STRING);
    type = stringType;
    genLCS(newPreDef(currentToken->string));	
	break;
  ...
  case TK_IDENT:
	...
	break;
	  case TP_FLOAT:
	type = floatType;
	genLCF(obj->constAttrs->value->floatValue);
	break;
	  case TP_STRING:
	type = stringType;
