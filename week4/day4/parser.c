  ...
  Type* varType;
	...
	switch(var->kind)
	{
		...
		case OBJ_FUNCTION:
			...
			break;
		default: error(ERR_INVALID_LVALUE,currentToken->lineNo,currentToken->colNo);
	}
...
void compileArgument(Object* param) {
  ...
  Type* type;
  if (param->paramAttrs->kind == PARAM_REFERENCE) type=compileLValue();
  else type=compileExpression();
    ...
    case OBJ_CONSTANT:
      switch(obj->constAttrs->value->type)
	  {
		...
		case TP_CHAR:
			type=charType;
			break;
		default: error(ERR_INVALID_BASICTYPE,currentToken->lineNo, currentToken->colNo);
	  }
	  break;
    case OBJ_VARIABLE:
