...
int compareType(Type* type1, Type* type2) {
  ...
  else if (type1->typeClass != TP_ARRAY) return 1;
  else if (type1->arraySize == type2->arraySize) return compareType(type1->elementType,type2->elementType);
  else return 0;
}

void freeType(Type* type) {
	if (type->typeClass==TP_ARRAY) free(type->elementType);
	free(type);
}
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {
  ConstantValue* value= (ConstantValue*) malloc(sizeof(ConstantValue));
  value->type = v->type;
  if (v->type==TP_INT) value->intValue= v->intValue;
  else value->charValue = v->charValue;
  return value;
}
...
void freeScope(Scope* scope) {
	if (scope->objList!=NULL) freeObjectList(scope->objList);
	free(scope);
}
...
Object* findObject(ObjectNode *objList, char *name) {
	while (objList!=NULL)
	{
		if (strcmp(objList->object->name,name)==0) return objList->object;
